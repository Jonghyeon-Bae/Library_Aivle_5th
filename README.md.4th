...
# 📚 Project Refactoring: PocketBase ➡️ Spring Boot & Cloud MySQL 전환 완료

## 본 프로젝트는 프로덕션 환경에서의 비즈니스 로직 안정성 향상과 백엔드 독립성을 확보하기 위해, 기존의 가벼운 BaaS(PocketBase) 단일 아키텍처 환경에서 **독립 백엔드(Spring Boot) + 클라우드 RDBMS(MySQL) 아키텍처**로 완벽하게 마이그레이션을 마쳤습니다.

## 🛠️ 주요 변경 및 개선 사항 (Key Updates)

### 1. 백엔드 및 데이터베이스 인프라 현대화

- **Spring Boot 백엔드 서버 구축:** Java 17, Gradle 환경의 백엔드 프로젝트(`library_5th_miniproject_backend`)를 전면 신설하여 독립 API 서버로 실행합니다.
- **MySQL & Aiven Cloud DB 연동:** 기존 SQLite(PocketBase 내장) 환경에서 고가용성 클라우드 DB인 **Aiven MySQL**로 마이그레이션했습니다.
- **보안 및 JWT 도입:** Spring Security와 JWT(Json Web Token)를 결합하여 사용자 인증 및 세션을 백엔드에서 제어하며, 토큰 기반의 무상태 API 통신 정책을 확립했습니다.

### 2. 프론트엔드 연동 아키텍처 개선

- **Axios 공통 클라이언트 & JWT 인터셉터:** PocketBase SDK를 폐기하고 Axios를 도입했습니다. 브라우저의 `localStorage`에 저장된 JWT 토큰이 존재하는 경우, 모든 요청에 `Authorization: Bearer <token>` 헤더를 자동 탑재하는 Request Interceptor를 구축했습니다.
- **API 레이어 계층화:** `bookApi`, `authApi`, `likeApi` 등으로 API 비즈니스 레이어를 완벽히 컴포넌트화하여 관리 효율성을 높였습니다.
- **데이터 규격 포맷 어댑터(Adapter) 적용:** Spring Boot의 `Page<Book>` 응답 스펙과 0-based page 번호 체계를 프론트엔드의 1-based page 번호 및 기존 UI 렌더링에 적합하도록 API 레벨에서 데이터를 가공하여 반환함으로써, 기존 프론트엔드 UI의 레이아웃 시프트와 코드 수정을 최소화했습니다.

---

## ⚙️ 실행 및 배포 구성

### 🖥️ 로컬 구동 (Local Running)

1. **백엔드 구동:** 백엔드 루트 폴더의 `.env` 파일에 데이터베이스 연결 정보를 기입한 뒤 Gradle 빌드 후 실행합니다. (Port: `8080`)
2. **프론트엔드 구동:** 프론트엔드 루트 폴더에 `NEXT_PUBLIC_API_URL=http://localhost:8080`을 설정한 뒤 `npm run dev`를 구동합니다. (Port: `3000`)

### ☁️ 클라우드 배포 (Deployment in Render)

- **Secret Files / Environment Variables 설정:**
  - 배포 플랫폼(Render 등)의 환경 설정 메뉴에서 `.env` 파일을 Secret File로 등록하거나 OS 레벨 환경변수에 `MYSQL_URL`, `MYSQL_USERNAME`, `MYSQL_PASSWORD` 등을 입력하여 백엔드 서버 기동 시 자동으로 인젝션되도록 배포 환경을 최적화했습니다.
